...
class PinLogin extends ConsumerWidget {
  ...
  Widget build(BuildContext context, ref) {
    final _pinControllers = List.generate(5, (i) => ref.watch(_pinControllerProvider(i)));
                  ...
                  Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                    ...
                    Row(
                      ...
                      children: [
                        for (int i = 0; i < 5; i++)
                          AppOtpField(
                            controller: ref.watch(_pinControllerProvider(i)),
                            onChanged: (value) {
                              if (value.isNotEmpty) {
                                if (i < 4) {
                                ...
                                final text = ref.read(_pinProvider.notifier).state;

                                // Concatenate the values in the textfields as one.
                                ref.read(_pinProvider.notifier).state =
                                (i < text.length)
                                    ? text.substring(0, i) + value + text.substring(i + 1)
                                    : text + value;
                              } else {
                                if (i > 0) {
                              ...
                              ref.read(enableButton.notifier).state =
                                  _pinControllers.every((controller) => controller.text.isNotEmpty);
                            },
                          ),
                      ],
                    ),
